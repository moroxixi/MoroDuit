/* ══════════════════════════════════════════════════════════════════════
   MoroDuit Opening — Script
   Fetch produk ditandai tag "b2b" di Katalog dan render
   ══════════════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  var loading = document.getElementById("loadingOpening");
  var container = document.getElementById("produkOpening");
  ...
  // ── Fetch & render ────────────────────────────────────────────────
  function loadProdukOpening() {
    loading.style.display = "block";

    var url = MORODUIT_CONFIG.APPS_SCRIPT_URL
      + "?action=getKatalogOpening"
    ...
    fetch(url)
      ...
      .catch(function (err) {
        loading.style.display = "none";
        section.style.display = "none";
        console.error("Fetch error (getKatalogOpening):", err);
      });
  }
